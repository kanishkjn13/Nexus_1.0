...
// Convert topics object → bar chart array
function topicsToChartData(topics) {
  return Object.entries(topics).map(([topic, accuracy]) => ({
    day: topic.charAt(0).toUpperCase() + topic.slice(1),
    xp: parseFloat(accuracy.toFixed(1)),
  }));
}

// Convert topics object → pie chart array
function topicsToPieData(topics) {
  const entries = Object.entries(topics);
  const total = entries.reduce((sum, [, v]) => sum + v, 0) || 1;
  return entries.map(([name, accuracy], i) => ({
    name: name.charAt(0).toUpperCase() + name.slice(1),
    value: parseFloat(((accuracy / total) * 100).toFixed(1)),
    xp: `${accuracy.toFixed(1)}%`,
    color: COLORS[i % COLORS.length],
  }));
}

// Convert topics object → learning progress list
function topicsToProgressList(topics) {
  const colorList = [
    'bg-purple-500', 'bg-blue-500',
    'bg-emerald-500', 'bg-amber-500',
    'bg-pink-500', 'bg-red-500',
  ];
  return Object.entries(topics).map(([name, accuracy], i) => ({
    name: name.charAt(0).toUpperCase() + name.slice(1),
    progress: Math.round(accuracy),
    color: colorList[i % colorList.length],
    tag:
      accuracy < WEAK_THRESHOLD ? '⚠️ Weak'
        : accuracy > STRONG_THRESHOLD ? '✅ Strong'
          : '📈 Medium',
  }));
}
...
function Card({ children, className = '', title, subtitle, extra }) {
  return (
...
function StatCard({ icon: Icon, title, value, change, subtitle, iconBg }) {
  return (
...
export function ProgressView() {
  ...
  const chartData = topicsToChartData(topics);
  const pieData = topicsToPieData(topics);
  const progressList = topicsToProgressList(topics);
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        >
          {chartData.length === 0 ? (
            <div className="flex items-center justify-center h-[300px] text-[#522B5B]/40 dark:text-[#DFB6B2]/30 font-bold text-sm">
            ...
            <div className="h-[300px] w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="xp" radius={[12, 12, 12, 12]} barSize={32} fill="url(#barGradient)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}

          <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/5 text-[11px] font-black uppercase tracking-widest">
        ...
        {/* XP Breakdown Donut — topic accuracy as pie */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Weekly Activity — topic accuracy as area chart */}
